...
function Server_sin_cola_av() {
    ...
    const handleCalculate = (e) => {
        ...
        const k = parseInt(inputs.k, 10); // Clientes para la tabla
        ...
        setResults({ 
            ...
            k: k // Incluir k en results para el encabezado de impresión
        });
    };

    // Componente reutilizable para las tarjetas de métricas (NUEVO DISEÑO)
    ...
    return (
        // Contenedor principal con max-w-6xl para aprovechar más el espacio y fondo oscuro
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="md:w-1/3 print:hidden">
                    <div className="bg-gray-800 p-6 rounded-xl shadow-2xl border border-gray-700 h-full">
                        ...
                        <p className="text-gray-400 text-center mb-6 text-sm">Línea de Espera: Cola Infinita</p>
                        
                        <form onSubmit={handleCalculate}>
                            <div className="space-y-4 mb-6">
                                <div>
                                    <label htmlFor="lambda" className="block text-sm font-medium text-gray-300 mb-1">Tasa de llegada ($\lambda$)</label>
                                    <input type="number" name="lambda" id="lambda" value={inputs.lambda} onChange={handleInputChange} placeholder="Ej: 8" className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"/>
                                </div>
                                <div>
                                    <label htmlFor="mu" className="block text-sm font-medium text-gray-300 mb-1">Tasa de servicio ($\mu$)</label>
                                    <input type="number" name="mu" id="mu" value={inputs.mu} onChange={handleInputChange} placeholder="Ej: 10" className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"/>
                                </div>
                                <div>
                    ...
                    {results && (
                        <div className="hidden print:block mb-6">
                            ...
                            <p className="text-center text-gray-600 mb-4">
                                Parámetros: $\lambda$ = {results.lambda} | $\mu$ = {results.mu} | Clientes (k) = {results.k}
                            </p>
                        </div>
                    )}
                    ...
                    {results ? (
                        <div className="space-y-8">
                            ...
                            <div className="bg-gray-800 p-6 rounded-xl shadow-2xl border border-gray-700 print:bg-white print:p-0 print:shadow-none print:border-none **break-inside-avoid**">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 print:gap-4 print:grid-cols-3">
                        ...
                        <div className="bg-gray-800 p-10 rounded-xl shadow-2xl border border-gray-700 text-center">
                            <p className="text-gray-400 text-lg">Ingresa los datos en el formulario para ver las métricas del sistema M/M/1.</p>
                        </div>
                    )}
                </div>
            </div>
