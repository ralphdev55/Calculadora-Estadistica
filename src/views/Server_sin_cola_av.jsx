...
function Server_sin_cola_av() {
    ...
    // Si el campo es 'lambda' y el valor es numérico y positivo, avanzamos al paso 2 automáticamente
    ...
    // Se conserva la lógica de cálculo, pero no se exponen controles de simulación.

    // --- Handlers para pop-up wizard (M/M/1: λ -> μ) ---
    const openWizard = () => {
        setModalStep(1);
    ...
    // Foco automático en modal cuando cambia el paso
    useEffect(() => {
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="md:w-1/3 print:hidden">
                    <div className="bg-gray-800 p-6 rounded-xl shadow-2xl border border-gray-700 h-full relative">
                        ...
                        {/* Modal wizard para entrada secuencial (λ -> μ) */}
                        {showStepModal && (
                            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                                <div className="absolute inset-0 bg-black/60" onClick={handleModalClose}></div>
                                <div className="relative bg-gray-800 rounded-xl p-6 max-w-md w-full shadow-2xl border border-gray-700 animate-scale-in">
                                    <h3 className="text-lg font-bold text-white mb-2">Paso {modalStep} de 2</h3>
                                    {modalStep === 1 && (
                                        <div>
                                    ...
                                    {modalStep === 2 && (
                                        <div>
                                            <div className="bg-gray-700/40 border border-gray-600 p-3 rounded-md mb-2 flex items-start gap-3">
                                                ...
                                                <div><p className="text-xs text-gray-200 font-medium">Explicación</p><p className="text-sm text-gray-300">μ es la tasa promedio de servicio por unidad de tiempo. Debe ser mayor que λ para estabilidad.</p></div>
                                            </div>
                                            <input ref={muRef} name="mu" type="number" step="any" placeholder="Ej: 10" value={inputs.mu} onChange={handleInputChange} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleModalNext(); } }} className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white" />
                                        </div>
                                    )}

                                    <div className="mt-4 flex justify-end gap-2">
                                        <button onClick={handleModalClose} className="bg-gray-600 hover:bg-gray-700 text-white py-2 px-4 rounded">Cancelar</button>
                                        <button onClick={handleModalNext} disabled={!isModalInputValid()} className={`py-2 px-4 rounded ${isModalInputValid() ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'bg-gray-600 text-gray-300 cursor-not-allowed'}`}>{modalStep < 2 ? 'Siguiente' : 'Calcular'}</button>
                                    </div>
                                </div>
                            </div>
                        )}
                                ...
                                {assistantActive && !showIntro && step === 1 && (
                                    <div className="absolute left-full ml-4 top-1/2 -translate-y-1/2 w-64 z-40 print:hidden">
                                        <div className="bg-emerald-600 text-white p-2 rounded-lg shadow-lg animate-float-up">
                                            ...
                                            <p className="text-xs mt-1">Introduce λ y presiona Enter para continuar.</p>
                                        </div>
                                    </div>
                                )}
                            </div>

                            {/* Mostrar el campo μ siempre (ahora ambos campos están visibles sin necesidad del botón) */}
                            <div className="relative">
